"""
Source: https://code.google.com/p/django-syncr/
...
class PicasawebSyncr:
    ...
    def getPhotoTagList(self, username, albumname, gphoto_id):
        ...
        feed = self.gd_client.GetFeed(
            '/data/feed/api/user/%s/album/%s/photoid/%s?kind=tag' % (username, albumname, gphoto_id))
    ...
    def _syncPhoto(self, photo_entry, username, albumname=None, refresh=False):
        ...
        taken_date = datetime(*strptime(photo_entry.timestamp.isoformat()[:-4] + '000Z', "%Y-%m-%dT%H:%M:%S.000Z")[:7])
        taken_date = timezone.make_aware(taken_date, timezone.get_current_timezone())
    ...
    def syncAlbum(self, album, username=None):
        ...
        updated = datetime(*strptime(album.updated.text[:-4] + '000Z', "%Y-%m-%dT%H:%M:%S.000Z")[:7])
        updated = timezone.make_aware(updated, timezone.get_current_timezone())
        ...
        d_album, created = Album.objects.get_or_create(gphoto_id=gphoto_id, defaults=default_dict)
        if self.cli_verbose:
            status = created and "created" or "already exists (same)"
